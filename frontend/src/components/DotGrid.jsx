...
const DotGrid = ({
  dotSize = 8,
  gap = 24,
  baseColor = '#313531',      // surface-800
  activeColor = '#a4c837',    // brand-500 (ivory)
  proximity = 120,
  speedTrigger = 80,
  shockRadius = 200,
  shockStrength = 4,
    ...
    const onMove = e => {
      ...
      for (const dot of dotsRef.current) {
        ...
        if (speed > speedTrigger && dist < proximity && !dot._animating) {
          ...
          const pushX = (dot.cx - pr.x) * 0.3 * falloff + vx * 0.003;
          const pushY = (dot.cy - pr.y) * 0.3 * falloff + vy * 0.003;
